...
function kzn.modules.load()
  ...
  vim.print('Parts', kzn.modules.parts)
  vim.print('Config', kzn.modules.config)

  vim.iter(kzn.modules.parts):each(function(module_path)
    local path, is_requireable = unpack(requirable.requirable(module_path))
    local ok, _module = kzn.pequire(module_path)

    if ok and is_requireable then
      vim.info('Path %s is requirable', path)
...
function kzn.modules.define(schema)
end
